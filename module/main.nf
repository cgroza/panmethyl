...
process annotate_BED {
  ...
  echo -e "QNAME\tCHROM\tSTART\tEND\tPATH\tFORMAT\t${sample}" > ${sample}.bed
...
process lift_nucleotides {
  ...
  script:
  """
  lift_nucleotides.py ${index} ${graph} | pigz > GFA_node_paths.bed.gz
  lift_edges.py ${index} ${graph} | pigz > GFA_edge_paths.bed.gz

  cat GFA_node_paths.bed GFA_edge_paths.bed.gz> GFA_paths.bed.gz
  """
}
