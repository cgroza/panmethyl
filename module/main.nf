...
process align_graphaligner
{
  ...
  cut -f1-12,17 ${sample_name}.gaf | sort -k1b,1 | pigz > ${sample_name}.gaf.gz
  rm ${sample_name}.gaf
  """
}
...
process align_minigraph {
  ...
  """
  samtools fasta --threads ${task.cpus} ${bam_path} | pigz  > ${sample_name}.fa.gz
  minigraph --vc -c -N 1 -t ${task.cpus} ${graph_path} ${sample_name}.fa.gz | cut -f1-12,19 | sort -k1b,1 | pigz > ${sample_name}.gaf.gz
  """
}
